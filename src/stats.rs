...
fn output_progress(stderr: &mut Stderr, bytes: usize, elapsed: String, rate: f64) {
    ...
    //send flush to make sure output makes
    let _ = stderr.flush();
}
...
impl TimeOutput for u64 {
    fn as_time(&self) -> String {
        ...
        // :02 to do zero padding for single digits
...
struct Timer {
    ...
    delta: Duration,     // now - last instant
    period: Duration,    //how often we want the timer to go off or be ready
    countdown: Duration, //how much time until the timer goes off next
    ready: bool,         // timer is ready
}
